...
bool match(const std::string& input_line, const std::string& pattern) {
    // Handle alternation (|)
    size_t pipePos = pattern.find('|');
    if (pipePos != std::string::npos) {
        std::string left = pattern.substr(0, pipePos);
        std::string right = pattern.substr(pipePos + 1);

        return match(input_line, left) || match(input_line, right);
    }
    ...
    do {
        if (match_here(input_line.substr(pos), pattern)) return true;
