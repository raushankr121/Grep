...
static std::vector<Token> parse_range(const std::string& pattern, size_t begin, size_t end, int& group_counter)
{
    ...
    while(i < end)
    {
        ...
        else if (pattern[i] == '[')
        {
            ...
            {
                if(pattern[close] == '\\' && close + 1 < e#include <iostream>
#include <string>
#include <cctype>
#include <stdexcept>
#include <vector>
...
void process_file(const std::string& filename, const std::string& pattern, bool& found_match, bool prefix_output) {
...
void process_directory(const std::string& dir_path, const std::string& pattern, bool& found_match) {
    try {
        for (const auto& entry : std::filesystem::recursive_directory_iterator(dir_path)) {
            if (entry.is_regular_file()) {
                process_file(entry.path().string(), pattern, found_match, true);
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    if (!E_found) {
        std::cerr << "Expected '-E' flag." << std::endl;
        return 1;
    }

    if (i >= argc) {
        std::cerr << "Pattern not provided." << std::endl;
        return 1;
    }
    pattern = argv[i++];
    ...
    try {
        if (recursive) {
            if (paths.empty()) {
                std::cerr << "Directory path not provided for recursive search." << std::endl;
                return 1;
            }
            if (paths.size() > 1) {
                std::cerr << "Recursive search takes only one directory path." << std::endl;
                return 1;
            }
            process_directory(paths[0], pattern, found_match);
        } else {
            if (paths.empty()) {
                ...
                bool prefix_output = (paths.size() > 1);
                for (const auto& path : paths) {
                    process_file(path, pattern, found_match, prefix_output);
                }
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " -E <pattern> [file1 file2 ...]" << std::endl;
        return 1;
    }
